...
export class CartService {
  ...
  public removeFromCart(productId: number, variant?: string) {
    this.items.set(
      this.items().filter(
        (i) => !(i.product.id === productId && i.selectedVariant === variant),
      ),
    );
    this.persist();
  }
  ...
  public get subTotal(): number {
    return this.items().reduce(
      (acc, item) =>
        acc + this.getUnitPrice(item.product, item.quantity) * item.quantity,
      0,
    );
  }
  ...
  public getUnitPrice(product: Product, quantity: number): number {
    ...
    try {
      const tiers = JSON.parse(product.wholesaleTiersJson || "[]") as {
        minQuantity: number;
        unitPrice: number;
      }[];
      return (
        tiers
          .filter(
            (t) =>
              t.minQuantity > 0 && t.minQuantity <= quantity && t.unitPrice > 0,
          )
