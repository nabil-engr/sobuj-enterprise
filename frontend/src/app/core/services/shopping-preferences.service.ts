...
export class ShoppingPreferencesService {
  ...
  isWishlisted(id: number): boolean { return this.wishlist().some(p => p.id === id); }

  toggleCompare(product: Product): void {
    const exists = this.compare().some(p => p.id === product.id);
    const next = exists ? this.compare().filter(p => p.id !== product.id) : [...this.compare(), product].slice(-4);
    this.compare.set(next);
    this.write(this.compareKey, next);
  }

  isCompared(id: number): boolean { return this.compare().some(p => p.id === id); }

  remember(product: Product): void {
    const next = [product, ...this.recentlyViewed().filter(p => p.id !== product.id)].slice(0, 12);
