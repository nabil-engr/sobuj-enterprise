...
export class AdminControlCenterComponent implements OnInit {
  ...
  load(path:string,target:any){this.http.get<any[]>(`http://localhost:5000/api/${path}`).subscribe({next:x=>target.set(x),error:()=>target.set([])})}
  ...
  emptyProduct(){return{title:'',slug:'',sku:'',shortDescription:'',description:'',price:0,discountPrice:null,stockQuantity:0,lowStockThreshold:5,categoryId:0,brandId:null,primaryImageUrl:'',isFeatured:false,isNewArrival:true,isBestSeller:false,rating:5,reviewCount:0,tags:''}}
  openProductForm(){this.editingProductId=null;this.productDraft=this.emptyProduct();this.formError.set('');this.productFormOpen.set(true)}
  editProduct(p:Product){this.editingProductId=p.id;this.productDraft={...this.emptyProduct(),...p,category:p.category,brand:p.brand};this.formError.set('');this.productFormOpen.set(true)}
  closeProductForm(){if(!this.savingProduct()&&!this.imageUploading())this.productFormOpen.set(false)}
  uploadImage(event:Event){const input=event.target as HTMLInputElement;const file=input.files?.[0];if(!file)return;if(file.size>5*1024*1024){this.formError.set('Image must be 5 MB or smaller.');return}const data=new FormData();data.append('file',file);this.imageUploading.set(true);this.http.post<{url:string}>('http://localhost:5000/api/Uploads/product-image',data).subscribe({next:r=>{this.productDraft.primaryImageUrl=r.url;this.imageUploading.set(false)},error:e=>{this.formError.set(e?.error?.message||'Image upload failed');this.imageUploading.set(false)}})}
  saveProduct(){this.formError.set('');if(!this.productDraft.title?.trim()||!this.productDraft.sku?.trim()||!this.productDraft.categoryId||this.productDraft.price<0){this.formError.set('Title, SKU, category and a valid price are required.');return}if(!this.productDraft.primaryImageUrl){this.formError.set('Please upload a product image or provide an image URL.');return}const payload={...this.productDraft,slug:this.productDraft.slug?.trim()||this.slug(this.productDraft.title),discountPrice:this.productDraft.discountPrice||null,category:undefined,brand:undefined};this.savingProduct.set(true);const request:any=this.editingProductId?this.productsApi.updateProduct(this.editingProductId,payload):this.productsApi.createProduct(payload);request.subscribe({next:()=>{this.savingProduct.set(false);this.productFormOpen.set(false);this.ok(this.editingProductId?'Product updated':'Product published');this.refresh()},error:(e:any)=>{this.savingProduct.set(false);this.formError.set(e?.error?.message||'Could not save product. Check SKU/slug uniqueness.')}})}
  createSimple(kind:string){const name=prompt(`${this.singular(kind)} name`);if(!name)return;const path=this.apiName(kind);const body=kind==='filters'?{name,code:this.slug(name).replaceAll('-','_'),displayType:'Checkbox',displayOrder:0}:{name,slug:this.slug(name),displayOrder:0,isFeatured:false};this.http.post(`http://localhost:5000/api/${path}`,body).subscribe(()=>{this.ok(`${name} created`);this.refresh()})}
  createTemplate(){const title=prompt('Template title');const templateType=prompt('Template type (unique)');const messageFormat=prompt('Message format with {OrderId}, {CustomerName}, {ItemsList}');if(!title||!templateType||!messageFormat)return;this.http.post('http://localhost:5000/api/WhatsAppTemplates',{title,templateType,messageFormat,isDefault:false}).subscribe(()=>{this.ok('Template created');this.refresh()})}
  editTemplate(t:any){const messageFormat=prompt('Edit message format',t.messageFormat);if(messageFormat===null)return;this.http.put(`http://localhost:5000/api/WhatsAppTemplates/${t.id}`,{...t,messageFormat}).subscribe(()=>{this.ok('Template updated');this.refresh()})}
  toggleRole(u:any){const role=u.role==='Admin'?'Customer':'Admin';if(!confirm(`Change ${u.fullName} to ${role}?`))return;this.http.patch(`http://localhost:5000/api/Users/${u.id}/role`,{role}).subscribe(()=>{this.ok('User role updated');this.refresh()})}
  remove(path:string,id:number){if(!confirm('This action permanently deletes the selected record. Continue?'))return;this.http.delete(`http://localhost:5000/api/${path}/${id}`).subscribe({next:()=>{this.ok('Record deleted');this.refresh()},error:e=>this.ok(e?.error?.message||'Delete failed: record may be in use')})}
