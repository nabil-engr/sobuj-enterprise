      ...
      <main class="flex-1 min-w-0 p-4 md:p-8">
        <div class="max-w-7xl mx-auto">
          ...
          @if(section()==='categories' || section()==='filters'){
            <div class="grid lg:grid-cols-[360px_1fr] gap-5"><form (ngSubmit)="saveSimple()" class="panel space-y-3"><div class="flex justify-between"><h3>{{simpleDraft.id?'Edit':'Create'}} {{singular(section())}}</h3>@if(simpleDraft.id){<button type="button" class="link" (click)="newSimple()">New</button>}</div><label>Name *<input required [(ngModel)]="simpleDraft.name" name="simpleName" placeholder="Display name"></label><label>URL slug / code *<input required [(ngModel)]="simpleDraft.slug" name="simpleSlug" placeholder="auto-generated if blank"></label>@if(section()==='categories'){<label>Description<textarea [(ngModel)]="simpleDraft.description" name="simpleDescription" rows="2"></textarea></label><label>Display order<input type="number" min="0" [(ngModel)]="simpleDraft.displayOrder" name="simpleOrder"></label><label class="flex-row items-center"><input type="checkbox" [(ngModel)]="simpleDraft.isFeatured" name="simpleFeatured"> Featured category</label>}@else{<label>Category<select [(ngModel)]="simpleDraft.categoryId" name="filterCategory"><option [ngValue]="null">All categories</option>@for(category of categories();track category.id){<option [ngValue]="category.id">{{category.name}}</option>}</select></label><label>Display type<select [(ngModel)]="simpleDraft.displayType" name="filterDisplay"><option>Checkbox</option><option>Dropdown</option><option>ColorSwatch</option></select></label><label>Display order<input type="number" min="0" [(ngModel)]="simpleDraft.displayOrder" name="filterOrder"></label>}<button class="primary w-full" [disabled]="simpleSaving()">{{simpleSaving()?'Saving...':simpleDraft.id?'Save changes':'Create'}}</button></form><div class="grid md:grid-cols-2 xl:grid-cols-3 gap-4">@for(x of currentSimpleItems; track x.id){<div class="panel"><div class="flex justify-between gap-3"><div><span class="material-symbols-outlined text-[#006c4e]">{{section()==='filters'?'tune':'category'}}</span><h3>{{x.name}}</h3><p class="text-xs text-slate-500 mt-1">{{x.slug||x.code}}</p><p class="text-[11px] text-emerald-700 mt-1">{{section()==='filters'?(x.displayType+' · '+(x.categoryId?'Category specific':'All categories')):(x.isFeatured?'Featured':'Standard')}}</p></div><div class="flex flex-col gap-2"><button class="link" (click)="editSimple(x)">Edit</button><button class="danger" (click)="remove(apiName(section()),x.id)">Delete</button></div></div></div>}</div></div>
          }

          @if(section()==='brands'){
            <div class="grid lg:grid-cols-[380px_1fr] gap-5"><form (ngSubmit)="saveBrand()" class="panel space-y-3"><div class="flex justify-between items-center"><h3>{{brandDraft.id?'Edit brand':'Add brand'}}</h3>@if(brandDraft.id){<button type="button" class="link" (click)="newBrand()">New</button>}</div><div class="h-28 rounded-xl border border-dashed bg-slate-50 flex items-center justify-center overflow-hidden">@if(brandDraft.logoUrl){<img [src]="brandDraft.logoUrl" class="h-full max-w-full object-contain p-3">}@else{<span class="text-xs text-slate-400">Brand logo preview</span>}</div><label>Brand name *<input required [(ngModel)]="brandDraft.name" name="brandName" placeholder="e.g. Double A"></label><label>Website<input [(ngModel)]="brandDraft.website" name="brandWebsite" placeholder="https://brand.com"></label><label>Logo image URL<input [(ngModel)]="brandDraft.logoUrl" name="brandLogo" placeholder="https://..."></label><label class="block bg-[#003527] text-white text-center rounded-xl p-3 cursor-pointer">Upload logo image<input type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml" class="hidden" (change)="uploadBrandLogo($event)"></label><label class="flex-row items-center"><input type="checkbox" [(ngModel)]="brandDraft.isFeatured" name="brandFeatured"> Show on storefront</label><button class="primary w-full" [disabled]="brandSaving()">{{brandSaving()?'Saving...':brandDraft.id?'Save brand':'Create brand'}}</button></form><div class="grid md:grid-cols-2 xl:grid-cols-3 gap-4">@for(brand of brands();track brand.id){<div class="panel"><div class="flex gap-3"><div class="w-16 h-16 rounded-xl bg-slate-50 border flex items-center justify-center shrink-0 overflow-hidden">@if(brand.logoUrl){<img [src]="brand.logoUrl" class="max-h-full max-w-full object-contain p-2">}@else{<span class="text-xs text-slate-400">No logo</span>}</div><div class="min-w-0"><h3 class="truncate">{{brand.name}}</h3><p class="text-[11px] text-slate-500 truncate">{{brand.website||'No website added'}}</p><p class="text-[11px] text-emerald-700 mt-1">{{brand.isFeatured?'Visible on storefront':'Hidden from storefront'}}</p></div></div><div class="mt-4 flex gap-3"><button class="link" (click)="editBrand(brand)">Edit</button><button class="danger" (click)="remove('Brands',brand.id)">Delete</button></div></div>}</div></div>
          }
...
export class AdminControlCenterComponent implements OnInit {
  ...
  productDraft:any=this.emptyProduct(); wholesaleTiers:{minQuantity:number;unitPrice:number}[]=[]; brandSaving=signal(false); brandDraft:any=this.emptyBrand(); simpleSaving=signal(false); simpleDraft:any=this.emptySimple();
  ...
  emptyBrand(){return{id:0,name:'',slug:'',logoUrl:'',website:'',isFeatured:true}}
  emptySimple(){return{id:0,name:'',slug:'',description:'',displayOrder:0,isFeatured:false,categoryId:null,displayType:'Checkbox'}}
  newSimple(){this.simpleDraft=this.emptySimple()}
  editSimple(item:any){this.simpleDraft={...this.emptySimple(),...item,slug:item.slug||item.code||''}}
  saveSimple(){const isFilter=this.section()==='filters';if(!this.simpleDraft.name?.trim()){this.ok('Name is required.');return}const slug=this.simpleDraft.slug?.trim()||this.slug(this.simpleDraft.name);const payload=isFilter?{...this.simpleDraft,code:slug.replaceAll('-','_'),slug:undefined}:{...this.simpleDraft,slug,code:undefined};this.simpleSaving.set(true);const path=isFilter?'Filters':'Categories';const request=this.simpleDraft.id?this.http.put(`${apiBaseUrl}/${path}/${this.simpleDraft.id}`,payload):this.http.post(`${apiBaseUrl}/${path}`,payload);request.subscribe({next:()=>{this.simpleSaving.set(false);this.ok(this.simpleDraft.id?'Changes saved':'Created');this.newSimple();this.refresh()},error:(e:any)=>{this.simpleSaving.set(false);this.ok(e?.error?.message||'Could not save. Check the name and unique slug.')}})}
  newBrand(){this.brandDraft=this.emptyBrand()}
  editBrand(brand:any){this.brandDraft={...brand}}
  uploadBrandLogo(event:Event){const file=(event.target as HTMLInputElement).files?.[0];if(!file)return;if(file.size>5*1024*1024){this.ok('Logo must be 5 MB or smaller.');return}const data=new FormData();data.append('file',file);this.brandSaving.set(true);this.http.post<{url:string}>(`${apiBaseUrl}/Uploads/product-image`,data).subscribe({next:r=>{this.brandDraft.logoUrl=r.url;this.brandSaving.set(false)},error:()=>{this.ok('Logo upload failed');this.brandSaving.set(false)}})}
  saveBrand(){if(!this.brandDraft.name?.trim()){this.ok('Brand name is required.');return}const payload={...this.brandDraft,name:this.brandDraft.name.trim(),slug:this.brandDraft.slug?.trim()||this.slug(this.brandDraft.name),website:this.brandDraft.website||'',logoUrl:this.brandDraft.logoUrl||''};this.brandSaving.set(true);const request=this.brandDraft.id?this.http.put(`${apiBaseUrl}/Brands/${this.brandDraft.id}`,payload):this.http.post(`${apiBaseUrl}/Brands`,payload);request.subscribe({next:()=>{this.brandSaving.set(false);this.ok(this.brandDraft.id?'Brand updated':'Brand created');this.newBrand();this.refresh()},error:(e:any)=>{this.brandSaving.set(false);this.ok(e?.error?.message||'Could not save brand. Check name and slug.')}})}
